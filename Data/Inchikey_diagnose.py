...
xyz_dir  = Path("/Users/guilingwei/chem_tools-4/src/chem_tools/preparation/xyz_outputs")  # *.xyz with InChIKey filenames
...
# ---- XYZ side: full keys and RootKeys from filenames ----
...
# Map RootKey -> set of full XYZ keys for quick lookup
root_to_full_xyz = (
...
# ---- Helper: try to resolve an LD row to match XYZ full keys under same RootKey ----
def resolve_to_xyz_key(smiles: str, target_full_keys: set[str]):
    ...
    # 1) If already matches (e.g., isomeric input), keep it
    ik0 = MolToInchiKey(m)
    if ik0 in target_full_keys:
        iso0 = Chem.MolToSmiles(m, isomericSmiles=True)
        return (iso0, ik0)

    # 2) Try assigning stereo as-is (perceive from 2D graph rules)
    m1 = Chem.Mol(m)
    ...
    # 3) Enumerate only unspecified stereocenters / E/Z and test
    try:
...
# ---- Identify mismatches to fix: same RootKey, different full key from XYZ ----
DF["needs_fix"] = False
DF["status"] = "unchanged"
...
for i, row in DF.iterrows():
    ik_ld  = row["InChIKey"]
    rk     = row["RootKey"]
    # If this LD InChIKey already exists on XYZ side, nothing to do
    if ik_ld in xyz_full_keys:
        DF.at[i, "status"] = "already_matched"
        continue

    # If we share a RootKey with any XYZ, try to resolve stereo to one of those keys
...
# ---- Build a final InChIKey column that prefers the updated key when present ----
...
# A merge ready to join directly with XYZ files
